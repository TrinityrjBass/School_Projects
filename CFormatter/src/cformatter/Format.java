...
//   parsing is used to parse the program and perform the formatting.
    ...
    private void forStatement() {
        ...
        verifyCurrentToken(Token.LEFT_PARENTHESIS);
        expression(Token.RIGHT_PARENTHESIS);        
        ...
        output.indent();//indent statement(s)
    ...
    private void doWhile() {
            ...
            output.endLine(false);
            output.indent(); 
            while(token != Token.RIGHT_BRACE && token != Token.WHILE){
                statement();
            }          
            verifyNextToken(Token.WHILE);
            expression(Token.SEMICOLON);
            token = lexer.getNextToken();
            output.unindent();
            output.endLine(false);
